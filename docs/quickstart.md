...
`title`/`body`/`entities`/`links` 现在用 CJK 感知的分析器（CJK 段切 unigram+bigram，拉丁词不变）。
**旧分片里的词条是旧分析器切的**，所以升级后要重建一次索引，否则中文词（如 `租约`）在老分片上仍然搜不到：

```bash
qm compact          # 租约保护的全量重建：从权威页面重新生成分片
```

`compact` 是幂等的、可重放；重建前后检索结果应当一致（`qm verify --strict` 可以复核桶的一致性）。
...
`qm search` 需要桶里已有分片：`qm publish` 之前检索不到，`qm maintain` 与 `qm compact` 同样会更新索引。
几件容易踩的事：

- **正向写入要等索引更新**，而 `recent` / `history` / `read-page` 直接读权威对象，**立刻**看得见它；
  `log` / `digest` 依赖建议性的 commit log，属 best-effort（日志写失败可能少一条）。
- 覆盖已发布页面的**新版本**在索引更新之前，那个 path 在 `search` 里会**暂时完全查不到**
  （旧版本已被权威判为过期），不是"查到旧内容"。
- **删除是例外**：`qm delete-page` 之后，权威 tombstone 立刻让旧索引命中失效，不需要先 publish。
...
新会话开始时，先问"上一个会话在做什么"，比上来就检索更快。这两条命令都**不读索引**（所以没发布过分片的机器
也得到同样的答案），但严格程度不同：`recent` 读 manifest 支撑的权威状态；`digest` 的 sessions / handoffs 段
来自权威对象，**pages 段来自建议性的 commit log**（提交后追加、写失败会被忽略），可能少一条记录。
...
**凭据边界的负向矩阵**（"错误签名必须被拒"、前缀级 ACL），以及 R2 的一致性、配额、延迟与错误 XML
变体仍未验证；凭据的桶作用域另有 2026-09-17 的观测（§10.9）。
