...
- **值不会被求值**：`$HOME` 就是字面量 `$HOME`，命令替换也不会执行（`QM_WRITER="$(hostname)"`
  会把字面量写进去，不要这么写）。凡是 shell 会读成**另一个值**的写法——双引号里的反斜杠转义、
  `"a"b` / `"a"#b` 这种引号拼接、`K=v w` 这种未加引号的第二个词——会**报错退出**而不是猜着用；
  未引用的反斜杠（如 `C:\tmp`）同样报错，用单引号包起来即可写字面反斜杠。
