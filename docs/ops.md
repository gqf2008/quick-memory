...
**结论：持续写入会把迁移饿住。** 转换的每一次尝试都要先把整份状态重新写成不可变分片
（本例 256 片，各一次对象写），再拿这些分片去 CAS 提交点；写者在那个窗口里总会先落一次提交，
于是转换在写入停下之前赢不了 CAS。本次转换在 200 次提交时开始，到第 400 次提交之后才提交
（重试 5 次）。所以它不是“慢一点”，而是**需要一个安静窗口**：运维上应当在写入低谷执行
`qm migrate-manifest`，或者接受它自己一直重试到安静为止。
