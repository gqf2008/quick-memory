...
- 一次 `qm digest` 的**三段都读整份对象**，读的对象数各自随对象数**线性**增长，并发上限统一是
  **16**，所以每段延迟约 `⌈N/16⌉ × RTT`，而不是 `N × RTT`：
  ...
  第二个数字要记；唯一不同的是它**不按 key 形状过滤**：`wal/` 前缀下每个对象都是一条记录，少读
  一个就是少一条记录，所以它读列出来的每一个 key，而缺失的对象是**失败**而不是跳过——并发的写方
  从不会删 WAL，那里没有「列了又没了」这种正常竞争。
- 上面这四批读**任一失败就整次失败**，不会返回部分结果；报出的是**listing 序里最小的那个 key**
  的失败，而不是先失败的那条，所以同一个桶坏掉几条对象时，每次报错都说同一个 key；`read_wal`
  复用同一个助手，它的错误身份同样由 listing 序而不是完成先后决定。
  唯独**会话被删**是容忍的：head 在 listing 之后被删除是快照读的正常竞争，digest 跳过它而不是
  整次失败。交接棒对象与 WAL 记录被删仍是失败——它们从来不是「列了又没了也照样算」的那种读。
