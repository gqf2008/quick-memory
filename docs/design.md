...
- **真 R2 账号**：已于 2026-09-16 在真 R2 上复验（§10.8）：`cas-conformance` `7/7 PASS`，并观测到
  PUT-only version。凭据的**桶作用域**后来另有一条观测（§10.9，2026-09-17）；这里仍然空着的是
  「错误签名必须被拒」的反向对照与**前缀级** ACL。
...
tantivy 的默认分词器按"非字母数字"切分，而**汉字在 Unicode 里属于字母类**：一整段没有标点的中文
（`潜在租约泄漏`）会变成**一个 token**，于是搜索 `租约` 命中 0 条——2026-09-16 在真桶上实测到的正是这个
（正文里 `租约` 出现两次，`qm search "租约"` 返回 `hits=0`；换成一整段反而能命中）。

因此 `title` / `body` / `entities` / `links` 四个字段改用 `qm-search::cjk` 注册的分析器（`cjk`）：

- **拉丁字母/数字**保持整词、转小写，超过 40 字节丢弃（与 tantivy `default` 的 `RemoveLongFilter` 同界，
  免得 base64 之类变成词条）；
- **CJK 段**同时产出每个字（unigram）与每对相邻字（bigram），位置都取首个字符的位置——
  unigram 让单字查询可用，bigram 让两三字词精确，位置连续保证短语查询可用；
- 索引与查询用**同一个**分析器，所以查询被切分的方式与它要找的文档完全一致。

**这套方案的语义**（免得后来者把有意的行为当 bug 修）：

- **bigram 是邻接守卫**：`租约泄漏` 只匹配连续出现的那一处；正文写成 `租约,泄漏`、`租约 泄漏`、
  `租约😀泄漏` 都不命中——跨标点/空白/emoji 的短语是刻意的假阴性，符合短语语义。
- **未加引号的长查询是 OR 语义**：`潜在租约泄漏` 会被切成 11 个字/词组条、按 `Occur::Should` 组合，
  所以召回偏宽、靠 BM25 排序；要"整段必须连续"得依赖 bigram 的邻接性，而不是"所有词都出现"。
- **代价**：CJK 段每个字与每对相邻字都进索引（约 2n 个词条），`的`/`是`/`在` 这类字会成为高频词，
  单字查询的候选集会明显变大——这是用"子串可搜"换来的，不是回归。

**升级与检测**：这是 schema 变更，旧分片里的词条仍是旧分析器切的。catalog 的
`IndexCatalog.schema` 现在标识分析器世代（1 = 默认分析器，2 = CJK 分析器），**读方（`search_project`）
...
**未发布的写入不在检索里**（这一点曾经被本文写成"有本地尾部，本机立即可见"，实现与那句话不符，
2026-09-16 核对后按事实改正）：

- `search` 只覆盖**已发布分片**。第 4 步的权威校验**只做过滤、不补页**，所以刚 `write_page`（或
...
6. **Linux 1.95 源码/测试套件已验证，Windows GNU 目标编译已验证，Windows 运行时仍未验证**：
   ...
   §10.8，那是在 macOS 上跑的）；仍未验证的是**真 embedding provider** 与凭据边界的**负向矩阵**
   （「错误签名必须被拒」、前缀级 ACL；桶作用域本身的观测见 §10.9）。
...
- 凭据：`QM_S3_*`（或 `R2_*`）从环境变量读，环境里没有则读本机配置文件
  （默认 `~/.quick-memory/env`，`QM_CONFIG_FILE` 可指定）；两处都没有就**报错**，不猜、
  不降级成本地存储。优先级固定为 命令行 > 环境变量 > 配置文件 > 内置默认。
...
- **中文召回**另有针对性回归：`a_chinese_word_inside_a_run_is_findable` 断言"正文含 `潜在租约泄漏` 时，
  搜 `租约`（以及单字 `租`、整段 `潜在租约泄漏`、无关词不命中）各得其所"。这条曾经失败过：
  默认分词器把整个 CJK 段当成一个 token，段内词一律搜不到（见 §6 的分析器一节）。
...
| 凭据泄露 | **终局口径**（§9 已拍板 A：每机一把 bucket-scoped token）：泄露一台机器 = 该桶可读写。缓解只有轮换与桶侧最小权限，步骤见 `ops.md`「凭据：签发 / 轮换 / 吊销」 |

## 9. 凭据与授权（**已拍板：方案 A —— 每机一把 bucket-scoped token**，2026-09-17）

> **决策已定：A。** 不做 Worker 网关：读与写都由机器持有自己的 bucket-scoped token 直连桶。
> §9.1–§9.4 是拍板时的对照分析（保留备查），§9.5 记录最终选择与代价，§9.6 是落地项，
> 操作步骤在 `ops.md`「凭据：签发 / 轮换 / 吊销」。
>
> 这个选择的代价：`writer_id` 永远只是自述（§9.3.4），凭据泄露等于该桶可读写
> （终局口径，缓解只有轮换与桶侧最小权限）。
...
- **A**：轮换 = 换 `S3_ACCESS_KEY_ENV_NAMES` / `S3_SECRET_KEY_ENV_NAMES` 里的值，每台机器
  各自更新；吊销 = 在桶侧吊销该 key。**隔离** = 一把 key 只覆盖一个桶。本节写作时说的是
  「本仓库对此没有证据」——**2026-09-17 补上了观测**（§10.9：在用的那把 token 列桶被拒、同账户
  另一个存在的桶也被拒、本桶 HEAD/List 正常），所以「每机一把 bucket-scoped key」在后端可行；
  **前缀级** ACL 仍是外部事实（S3/R2 的 IAM 细节）。本节沿用既有口径
...
- **选择 A（最终决定）** 必须接受：`writer_id` **永远**只是自述（§9.3.4），且「凭据泄露 →
  该桶可读写」是终局口径；缓解只有轮换与桶侧最小权限。「每机一把 bucket-scoped key 是否可行」
  现在有了本仓库观测（§10.9，2026-09-17）：在用的那把 token `ListBuckets` 被拒、同账户**另一个
  确定存在的桶**也被拒，而该桶内 HEAD/List 正常（写路径另有 §10.9 的写入冒烟；签发与验证步骤见
  `ops.md`「凭据」）。

**最终选择（2026-09-17）：A —— 每机一把 bucket-scoped token。** 本节给出的建议（C1）**未被采纳**：
被放弃的是「泄露一台机器 ≠ 泄露全桶」与「提交点上的可验证归因」，换来的是不引入新组件、
读路径的离线性不受影响、写路径不多一跳。落地项见 §9.6。

### 9.6 决策后的落地项

**A 的三条（已落地）：**

1. **每机一把 bucket-scoped key 的可行性与步骤** —— 步骤在 `ops.md`「凭据：签发 / 轮换 / 吊销」；
   本仓库观测（§10.9，2026-09-17）：在用的 token 列桶被拒、同账户另一个存在的桶也被拒、本桶 HEAD/List 正常
   （写路径：2026-09-17 用「清掉所有 S3 凭据环境变量、凭据只来自配置文件」的写入冒烟 + aws 侧对象
   核验证明，见 §10.9）。
   「恰好只授权这一个桶」那一半来自控制台 Scope 配置，不是 API 证明（§10.9 边界）。
2. **轮换与吊销手册** —— 同上，含换发顺序（**先发新、再撤旧**，避免写路径断档）与吊销后的表现。
3. **「`writer_id` 是自述」写进 docs** —— 见 §9.3.4 与 `ops.md`「归因边界：`writer_id` 是自述，不是认证」。

**以下是拍板时的原始工作项草稿，保留备查**：被采纳的 A 已落在上面的「A 的三条」，下面是未采用的 C1 / C2。
...
**若选 A（已采纳；下面是拍板时的原始草稿，落地情况见上面的「A 的三条」）：**

1. 把「每机一把 bucket-scoped key 是否可行」写成后端的**实测项**——草稿里写的是「本仓库没有证据」，
   那是拍板前的状态；2026-09-17 已补上观测（§10.9）。
2. 在 `ops.md` 补轮换与吊销的操作手册。——已做。
3. 把「`writer_id` 是自述」写进 `docs/`，避免它被当审计依据。——已做。
...
原为 C1/C2 而列的两项——Workers 的 R2 binding 条件写语义、presigned URL 与 `If-Match` 的组合——
**随选 A 一并作废**：A 没有 Workers binding，也没有 presigned URL。
...
## 10.5 真实 S3 实现复验（MinIO，2026-09-16 @ c9006a0）

2026-09-16 在 `main` 的 `c9006a0` 上，用一次性本地 MinIO 容器重新跑过下列链路，全部通过。
MinIO 是**真实的 S3-compatible HTTP 后端**，不是内存后端；但它**不是 R2**，下面的数字不能证明
R2 的 ACL、签名校验、版本行为、一致性或错误 XML 变体。

复现命令使用占位符，不把凭据或真实 scope 写进仓库：

```bash
export QM_S3_ENDPOINT="http://127.0.0.1:9100"
export QM_S3_BUCKET="<minio-bucket>"
export QM_S3_ACCESS_KEY_ID="<access-key>"
export QM_S3_SECRET_ACCESS_KEY="<secret-key>"
export QM_S3_REGION="<region>"
...
# 向量闭环；QM_EMBEDDING_* 指向本地 deterministic HTTP stub，不是真 provider
cargo run -p qm-probe --bin search-probe -- vector-publish --workspace <workspace> --project <project> <pages.jsonl>
cargo run -p qm-probe --bin search-probe -- vector-query --workspace <workspace> --project <project> "<query>"

qm maintain --json
qm search "<query>" --json
qm verify --strict
```

| 验证 | 2026-09-16 @ `c9006a0` 原始结果 |
|---|---|
| `cas-conformance`（条件写契约） | `7/7 PASS`；MinIO 的 PUT 返回 `version=None`，ETag 路径正常 |
| `manifest-probe --machines 3 --writes 5` | `commits=15 attempts=19 pages=3 wal=15 all checks passed` |
| `search-probe project` | `splits=3 hits=1 filtered_out=1 all checks passed` |
| `session-probe` | `observations=3 segments=2 splits=1 hits=1 all checks passed` |
| `digest-probe seed` / `read` | 显式唯一 `workspace` / `project`，两个独立进程；`read`: `pages=2`（`page_deleted=1`）、`sessions=1`、`handoffs=2`、`live_pages=1`；删除的 path 在 digest 里、不在 `live_pages` 里 |
| `search-probe vector-publish` / `vector-query` | 两个独立进程；查询与正文无词法重合，`streams_active=["vector"]`、`stream_candidates.vector=1`，命中 `notes/semantic.md`；同宽换 `vector-model-b` 被 identity guard 拒绝。embedding 来自本地 deterministic HTTP stub，不是真 provider |
| `qm maintain`（第一次） | `consolidated=1 published=true manifest_seq=1 splits=1` |
| `qm maintain`（第二次） | `consolidated=0 already_up_to_date=1 published=false`；随后 `qm search` 命中 1 条 |
| `qm verify --strict` | 通过（`no problems`） |

**边界**：这组证据覆盖 MinIO 上的真实 HTTP/S3-compatible 读写路径；不覆盖 R2 特有的 PUT-only version、
GET 不返回 version、ACL/凭据边界、服务端签名拒绝、区域/一致性/延迟/配额/错误 XML 变体。
向量行的 provider 是本地 deterministic HTTP stub，不是真 provider。`qm maintain` 的两次运行验证的是
...
- 向量链的 embedding 来自**本地 deterministic HTTP stub**，不是真 provider：真 provider 的网络、模型与
  维度行为仍未验证（§6.20）；
- **这次没有做 ACL / 凭据边界测试**（全程一把 token），也没有"错误签名必须被拒"的反向对照
  （凭据的桶作用域另有一条独立观测，见 §10.9；错误签名与前缀级 ACL 仍然空着）；
- 该桶不是 quick-memory 专用桶（同时存在其它项目的对象），因此每次运行都用唯一的 workspace / prefix
  隔离，验证结束后清理了本次自建的对象；
- §5.1 列出的其余 R2 行为（延迟、配额、区域、错误 XML 变体）依旧没有观测。

## 10.9 R2 凭据作用域观测（2026-09-17 @ 1739c1b）

2026-09-17 在持有该桶凭据的机器上（主工作树 `main @ 1739c1b`，0.2.0 发版之后）做了一组**手工**观测，
只回答一个问题：§9 选的「每机一把 bucket-scoped token」，**泄露半径**到底是不是「这一个桶」。
命令用 `aws` CLI 直连同一个 S3 端点（占位符；凭据、账户与真实对象键都不写进仓库）：

```bash
aws s3api list-buckets    --endpoint-url https://<account>.r2.cloudflarestorage.com
aws s3api head-bucket     --bucket <bucket>     --endpoint-url …
aws s3api list-objects-v2 --bucket <bucket> --max-items 1 --endpoint-url …
aws s3api head-bucket     --bucket git-gqf2008  --endpoint-url …   # 同账户下另一个**确定存在**的桶
aws s3api head-bucket     --bucket <不存在的桶名> --endpoint-url …
```

| 观测 | 原始结果 |
|---|---|
| `list-buckets`（账户级操作） | `An error occurred (AccessDenied) when calling the ListBuckets operation: Access Denied`，exit 254 |
| `head-bucket` 本桶 | exit 0 |
| `list-objects-v2` 本桶（`--max-items 1`） | exit 0，返回该桶内的真实对象 |
| `head-bucket` 同账户另一个**确定存在**的桶 | `An error occurred (403) when calling the HeadBucket operation: Forbidden`，exit 254 |
| `head-bucket` 一个不存在的桶名 | 同样是 `403 Forbidden`，exit 254 |

**结论**：这把凭据**不是账户级**（账户级列举被拒），并且在观测范围内**够不到别的桶**——同账户下
另一个确定存在的桶（`git-gqf2008`，本机 walgit 服务正在用它）也被拒。加上签发时**在控制台把 Scope
指定到这一个桶**这个配置事实（`ops.md` 签发第 2 步），「泄露一台机器 = 该桶、而不是整个账户」成立。

**边界**（这组观测不覆盖什么）：

- 后两行说明 R2 对「桶不存在」与「桶存在但无权限」都回 403，**单看 403 区分不出两者**；真正有区分力的
  是「账户级列举被拒」＋「一个确定存在的桶也被拒」。要独立证明「**恰好**授权了一个桶」得逐桶枚举，
  API 侧做不到——所以「只授权这一个桶」这一半来自**控制台配置**，不是 API 证明。这里能说的是：在
  观测范围内，该凭据可用的只有这个桶；
- 上面这组观测只走**读侧**（`HEAD` / `List`），没有写操作；**写路径**见下面这次的写入冒烟。另有两条
  更弱的补充：「§10.8 那次真 R2 复验的写全部走同一个配置文件提供的凭据」，以及该配置文件 mtime
  （2026-09-16 15:30）之后桶里仍有**本机写入**的新对象（最新 `2026-09-17T04:09:00Z` 的索引
...
- **写路径冒烟（2026-09-17 当天补做）**：先把本进程里所有 `QM_S3_*` / `R2_*` / `AWS_*` 变量用
  ...
  `…/pages/ops/r2-credential-scope.md/versions/<page_id>.md`（1889 B，
  `LastModified 2026-09-17T06:31:12Z`）与同一秒的 `wal/<page_id>.json`、`manifest.json`、
  `commits/00000000000000000006.json`。这一页就是冒烟的产物
  （`qm read-page --path ops/r2-credential-scope.md`）。它证明的是**当前配置文件里这把凭据**的写路径
  可用；§10.8 的写与 mtime 旁证只作补充；
- 换 token 之后要按 `ops.md` 轮换步骤第 3 步重跑本节的观测（读侧）与写入冒烟（写侧）；
- 这是**操作侧手工观测**（在持凭据的机器上跑 `aws`），不是仓库里可复现的探针，也没有进 CI；复现要有
  凭据，步骤见 `ops.md`「凭据：签发 / 轮换 / 吊销」；
- 没有证明：**前缀级** ACL、桶级策略边界、跨账户越权矩阵、「错误签名必须被拒」的反向对照——这些仍然
  空着（§10.8 边界、§11）。
...
- 鉴权/凭据方案**已拍板：A —— 每机一把 bucket-scoped token**（2026-09-17）。代价（自述归因、
  泄露面 = 整个桶）与落地项见 §9.5/§9.6；签发、轮换、吊销步骤见 `ops.md`。
...
**真 embedding provider**（向量链跑的是本地 deterministic stub）与**凭据边界的负向矩阵**
（"错误签名必须被拒"、前缀级 ACL；§10.8 全程用一把 token）。凭据的**桶作用域**本身另有一条
2026-09-17 的观测（§10.9）。下表保留每一项各自的证据层级：
