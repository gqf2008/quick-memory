...
- **窗口按各自的时钟**：pages 按提交时间、sessions 按 head 的 `updated_at_ms`、
  ...
  没有变化是**空 vec**，不是错误。
- **`limit` 保留窗口内最新的那端**：先按窗口过滤，再按 `at_ms` 降序、`seq` 破平局排序，然后截断。
  `at_ms` 是**调用方提供的时钟**，所以在时钟偏斜下"提交顺序"与"时间顺序"可能不一致——这里的契约是时间顺序，
  而 `seq` 作为 tie-breaker 让顺序成为**全序**：两台机器读同一份日志必须对顺序一致，而不只是对集合一致。
