...
- **选择 A（最终决定）** 必须接受：`writer_id` **永远**只是自述（§9.3.4），且「凭据泄露 →
  ...
  **现在有了本仓库证据**：在用的这把就是桶作用域的——`ListBuckets` 被 403 拒绝，而该桶内对象读写正常
  （签发与验证步骤见 `ops.md`「凭据」）。
...
1. **每机一把 bucket-scoped key 的可行性与步骤** —— 步骤在 `ops.md`「凭据：签发 / 轮换 / 吊销」；
   本仓库证据：在用的 token 是桶作用域的（`ListBuckets` → 403，桶内读写正常）。
...
A 之下需要实测的是「桶作用域的最小权限 token 足够跑通全部路径」，这一条**已在真 R2 上验证**：
`cas-conformance`、并发提交、跨分片检索、删除/压缩、format 2 迁移回滚等全部通过，见 §10.8。
凭据本身曾经不阻塞协议验证，现在也不阻塞部署。
