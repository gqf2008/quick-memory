...
- **A**：轮换 = 换 `S3_ACCESS_KEY_ENV_NAMES` / `S3_SECRET_KEY_ENV_NAMES` 里的值，每台机器
  ...
  「本仓库对此没有证据」——**2026-09-17 补上了观测**（§10.9：在用的那把 token 列桶被拒、
  别的桶名被拒、本桶读写正常），所以「每机一把 bucket-scoped key」在后端确实可行；
...
- **选择 A（最终决定）** 必须接受：`writer_id` **永远**只是自述（§9.3.4），且「凭据泄露 →
  ...
  现在有了本仓库观测（§10.9，2026-09-17）：在用的那把 token `ListBuckets` 被拒、**别的桶名**
  也被拒，而该桶内对象读写正常（签发与验证步骤见 `ops.md`「凭据」）。
...
1. **每机一把 bucket-scoped key 的可行性与步骤** —— 步骤在 `ops.md`「凭据：签发 / 轮换 / 吊销」；
   本仓库观测（§10.9，2026-09-17）：在用的 token 列桶被拒、别的桶名被拒、本桶读写正常。
...
别的桶名被拒、本桶读写正常。仍然空着的是**前缀级 ACL** 与「错误签名必须被拒」这类负向矩阵
（§10.8 边界、§11）。凭据本身不阻塞协议验证，也不阻塞部署。
...
2026-09-17 在持有该桶凭据的机器上（主工作树 `main @ 1739c1b`，即 0.2.0 发版之后）做了一次**手工**观测，
只回答一个问题：§9 选的「每机一把 bucket-scoped token」在后端真的成立吗。命令用 `aws` CLI 直连同一个
S3 端点（占位符；凭据、账户与真实对象键都不写进仓库）：
...
aws s3api head-bucket   --bucket <另一个桶名> --endpoint-url …
```
...
| `head-bucket` 另一个桶名 | `An error occurred (403) when calling the HeadBucket operation: Forbidden`，exit 254 |

**结论**：这把凭据是**桶作用域**的——它能读写本桶，却列不出账户里的桶，也够不到别的桶名。这正是 §9
「A 的代价 = 泄露一台机器等于该桶可读写」的实证前提，也是 §9.6 工作项①要的那条依据。

**边界**（这组观测不覆盖什么）：

- 这是**操作侧手工观测**（在持凭据的机器上跑 `aws`），不是仓库里可复现的探针，也没有进 CI；复现要有
  凭据，步骤见 `ops.md`「凭据：签发 / 轮换 / 吊销」；
- 它只证明**桶作用域**这一条。**没有**证明：**前缀级** ACL、桶级策略边界、跨桶/跨账户的完整越权矩阵、
  「错误签名必须被拒」的反向对照——这些仍然空着（§10.8 边界、§11）；
- 第 4 行是**辅证**：R2 对「不存在的桶」是否也一律回 403 没有单独验证，所以「别的桶名被拒」不能单独
  区分「凭据被限制」与「桶不存在」；主证是 `list-buckets` 被拒而本桶可读写；
- 结论与**这把 token** 绑定：轮换之后要重测（`ops.md` 轮换步骤的第 4 步就是这件事）。
