...
- **选择 A（最终决定）** 必须接受：`writer_id` **永远**只是自述（§9.3.4），且「凭据泄露 →
  ...
  确定存在的桶**也被拒，而该桶内 HEAD/List 正常（写路径另见 §10.8 与本机实际落盘；签发与验证
  步骤见 `ops.md`「凭据」）。
...
1. **每机一把 bucket-scoped key 的可行性与步骤** —— 步骤在 `ops.md`「凭据：签发 / 轮换 / 吊销」；
   ...
   （写路径：§10.8 的写全部走同一个配置文件提供的凭据，且本机在配置文件 mtime 之后仍有新对象落盘）。
...
同账户另一个确定存在的桶也被拒，本桶 HEAD/List 正常，且本机在配置文件 mtime 之后仍有新对象落盘。
...
- 后两行说明 R2 对「桶不存在」与「桶存在但无权限」都回 403，**单看 403 区分不出两者**；真正有区分力的
  ...
  观测范围内，该凭据可用的只有这个桶；
- 这组观测只走**读侧**（`HEAD` / `List`），§10.9 没有写操作。**写路径**的证据在别处：§10.8 那次真 R2
  复验的全部写操作都走同一个配置文件提供的凭据，而且该配置文件 mtime（2026-09-16 15:30）之后，桶里
  仍有**本机写入**的新对象（最新 `2026-09-17T04:09:00Z` 的索引 `head.json` / `catalog` / splits）。
  这两条只是**旁证**：mtime 只说明配置文件最后一次被改的时间，不是凭据内容的指纹；对象时间只说明
  「那时发生过写」，不能单独证明写操作用的就是该文件里**当前版本**的凭据（若当时 export 了
  `QM_S3_ACCESS_KEY_ID` / `QM_S3_SECRET_ACCESS_KEY`，凭据来源其实是环境变量）。要强证明就得在
  「没有旧环境变量、明确只用新配置」的前提下补一次写入冒烟；换 token 之后要按 `ops.md` 轮换步骤
  第 3 步重跑本节的观测并复核写路径；
