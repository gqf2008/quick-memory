...
本地 deterministic stub）与**凭据边界的负向矩阵**（"错误签名必须被拒"、前缀级 ACL）。凭据的桶作用域
有一条 2026-09-17 的观测（§10.9）。Quickwit 官方 0.9.0 容器产出的真分片读取验证见 §6.4。
...
```bash
# 下面两行是「一次性 shell 注入」的写法（一行多个赋值）。
# 写进 ~/.quick-memory/env 时请**每行一个键**——示例见 quickstart 的「配置文件」；
# qm / qm-mcp 会自己读该文件，优先级：命令行 > 环境变量 > 配置文件 > 内置默认。
