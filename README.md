...
crates/qm-mcp     `qm-mcp` MCP stdio 服务器（24 个 memory_* 工具）
```
...
`memory_handoff_claim` / `memory_handoff_done` / `memory_history` / `memory_restore` / `memory_log` /
`memory_recent`。它们与 `qm` 命令共用同一份实现，返回 JSON。
